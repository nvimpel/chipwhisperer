...
### Reprogramming the USB Interface (SAM3U)
...
In addition to programming the FPGA over the USB interface, the CW305 can additionally be programmed
over JTAG from Xilinx Vivado software. With both of these methods,
the CW305 must be reconfigured every time the device is power cycled.
...
This will generate a file called `ChipWhisperer-CW305-SAM3U1C.bin` you can program in, or `ChipWhisperer-CW305-SAM3U1C.bin` you can load in something like 
[Ozone](https://www.segger.com/products/development-tools/ozone-j-link-debugger/) if you have a debugger attached.

### Re-Programming SAM3U via Python

```Python
import chipwhisperer as cw

# or CW305 target object
programmer = cw.SAMFWLoader(scope=fpga)
